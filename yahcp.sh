...
python3 "$current_dir"/fraglist.py --fasta "$fasta" --enzyme "$enzyme" --output-dir "$output_dir" --size "$size""$circular"
...
python3 "$current_dir"/fraglist.py --intersection "$tmp_dir"/contact_intersect_sorted.bed --frags "$output_dir"/fragments_list.txt --output-dir "$output_dir" ${posmat:+-p}
